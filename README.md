...
Recipe validation plugin for the `wk` CLI. Validates Workato recipe JSON files against 66 built-in rules across four tiers of analysis, from basic JSON schema checks through cross-step data flow validation. Runs as a standalone command or as an automatic pre-push gate on `wk push`.
